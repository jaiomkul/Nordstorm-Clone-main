...
async function getData() {
  try {
    let res_user = await fetch(
      "https://nordstrom-backend-production.up.railway.app/users"
    );
    ...
    let res = await fetch(
      "https://nordstrom-backend-production.up.railway.app/cart/"
    );
...
const appendCart = (data, user_id) => {
  ...
  data.forEach((elem) => {
    ...
    if (elem.user_id == loger_id) {
      ...
      delivery_img.src =
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSaXluXZ-lyx9-SF9ldyZY_OndWZpm0L1UvHg&usqp=CAU";
...
async function removeData(id) {
  try {
    let res = await fetch(
      `https://nordstrom-backend-production.up.railway.app/cart/${id}`,
      {
        method: "DELETE",
        headers: {
